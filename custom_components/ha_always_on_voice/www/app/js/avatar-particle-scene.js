 ...
 *
 * Ported from pq_dash_voice's particle-scene.js for the "avatar" animation style.
 * Differences from the native-app original: sized against a container element
 * (this.sizeTarget) instead of window.innerWidth/innerHeight, since here the canvas
 * lives inside a fixed-size UI panel rather than filling the whole screen; a
 * ResizeObserver replaces the window resize listener for the same reason; and a
 * dispose() method was added so switching the animation style away from "avatar"
 * at runtime can free the WebGL context instead of leaking it.
 *
 ...
 * frame, which keeps this cheap enough to run continuously on a 24/7 device.
 *
 * States: IDLE, ASSEMBLING, LISTENING, THINKING, SPEAKING, SUCCESS, ERROR.
 * setState()/setAudioLevel() form the same contract the original Kotlin bridge used;
 * window.particleInterface (if present) is still notified so the assembly-complete
 * signal can be observed the same way.
 */
...
class ParticleScene {
    ...
    constructor(canvasElement, sizeTarget = canvasElement.parentElement) {
    ...
    autoDetectParticleCount() {
        ...
        const pixels = this.sizeTarget.clientWidth * this.sizeTarget.clientHeight * pixelRatio;
    ...
    /** Frees the WebGL context and stops observing/animating. Safe to call more than once. */
    dispose() {
        this.isRunning = false;
